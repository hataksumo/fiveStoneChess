...
Shader "Custom/ItemSpecular" {
	Properties {
		albedoMap ("Albedo Map (RGB)", 2D) = "white" {}
		specularMap ("Specular Map (RGB)", 2D) = "white" {}
		...
		specularPower("Specular Power", Float) = 8
	}
	SubShader {
		Tags {
			"RenderType" = "Opaque"
		...
		Pass {
			...
			float4 PFunc(V2P i) : COLOR {
				...
				float4 specularTex = tex2D(specularMap, i.texCoord.xy);
				float3 albedo = pow(albedoTex.xyz,1);
				float3 normal = normalize(i.normal);
				float diffuse = saturate(dot(normal, -i.viewDir));
				float specular = pow(dot(i.viewDir,normalize(reflect(normal,-i.viewDir))),specularPower);
				...
				float3 color = ambientS/2*(ambientRange +normal.y*(1-ambientRange))*albedo+ cameraS/2*diffuseColor;// +
				float3 fresnel = (1-max( dot( -vdirFresnel, normal ), 0 ))*tintColor.xyz;
				color =  color + fresnelColor/3 + pow(fresnel,1)+selectColor;
				color.rgb = color.rgb+specular*specularTex.rgb;
