...
Shader "Custom/CharacterSimple" {
	Properties {
		...
		campColor ("Camp Color", Color) = (0, 0, 0, 0)
	...
	SubShader {
		...
		Pass {
			...
			float4 PFunc(V2P i) : COLOR {
				...
				float4 maskTex = tex2D(maskMap, i.texCoord.xy);
				...
				color = lerp(gray*campColor.rgb,color,maskTex.r);
